...
export async function resolveNpubService(npub, serviceId = 'api', options = {}) {
    console.log(`[Resolver] Resolving ${npub} for service ${serviceId}...`);
    const { torPreferred = true, allowedProtocols = ['wss', 'ws', 'https', 'http', 'tcp'], nsec = options.nsec || process.env.NSEC, bootstrapRelays = DEFAULT_RELAYS } = options;
    if (nsec) {
